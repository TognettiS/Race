...
public class CarAI : MonoBehaviour
{
    ...
    private void PathProgress() //Checks if the agent has reached the currentWayPoint or not. If yes, it will assign the next waypoint as the currentWayPoint depending on the input
    {
        ...
        void ListOptimizer()
        {
            if (currentWayPoint > 1 && waypoints.Count > 10)
            {
    ...
    {
        frontLeft.brakeTorque = 400;
        frontRight.brakeTorque = 400;
        backLeft.brakeTorque = 400;
        backRight.brakeTorque = 400;
    }
    ...
    void ApplySteering() // Applies steering to the Current waypoint
    {
        ...
        if (SteeringAngle > 70) LocalMaxSpeed = 50;
    ...
    void Movement() // moves the car forward and backward depending on the input
    {
        ...
        if (allowMovement == true)
        {
            ...
            {
                backRight.motorTorque = 100 * MovementTorque;
                backLeft.motorTorque = 100 * MovementTorque;
                frontRight.motorTorque = 100 * MovementTorque;
                frontLeft.motorTorque = 100 * MovementTorque;
            }
            else if (SpeedOfWheels < LocalMaxSpeed + (LocalMaxSpeed * 1 / 4))
            {
                backRight.motorTorque = 10;
                backLeft.motorTorque = 10;
                frontRight.motorTorque = 10;
                frontLeft.motorTorque = 10;
            }
            else
                ApplyBrakes();
